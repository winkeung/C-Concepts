...
# **(Arrays are second class citizens in C. They cannot do value assignment with the same kind as other data types can do. Pointers steal their operator [] and pretend to be arrays. Arrays have to lie to people that they are pointers (the exact type of pointer who steal [] from them) in order to use back their own operator[]. i.e. They need to pretend to be somebody who pretend them in order to pretend back themselves. Only sizeof() and '&' (address of operator) can identify them.)**
