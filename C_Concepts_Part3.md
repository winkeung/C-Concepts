...
# **(Array is second class citizen in C. It cannot do value assignment with the same kind as other data types can do. Pointer steal its operator []. She has to tell people she is a pointer when she is asked. By disguising as pointer she can then use back its own operator[].)**
